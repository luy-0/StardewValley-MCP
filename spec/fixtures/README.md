...
`actionFixtures` 为 V1 的十一项变更能力各保留一个聚合文件。每个文件固定最小输入、完整输入、非法输入、`ACCEPTED` 检查点、成功 Tool 结果和失败 Tool 结果；这样可以覆盖能力契约，又不会演变成每项能力六个平铺文件。`navigate`、`interact`、`use_tool` 与 `transfer_inventory_item` 的文件还固定各自的长时执行门禁与失败语义。
