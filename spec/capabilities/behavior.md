...
`CropFact` 的新增字段描述当前作物实例的语言无关生长事实。`has_fertilizer` 明确区分已确认有、已确认无与字段缺失；为 true 且肥料身份可安全解析时提供 `fertilizer_item_id`，为 false 时不得提供该 ID。`growth_phase` 是从 0 开始的当前阶段序号，`growth_phase_count` 包含最终成熟阶段；未成熟时可以用 `growth_phase_day/growth_phase_duration` 表达当前阶段内已完成日数与所需日数。`growth_days_remaining_if_watered` 只承诺在之后每个需要浇水的生长日都满足条件时，距离首次成熟还需的生长日，不能解释成自然日、日历日期或必然收获日期。`regrow_days` 是可再生作物的基础收获间隔，`regrow_days_remaining` 只在已经成熟并进入再生倒计时时出现。`mature=true` 表示作物活着且已经到达最终生长阶段，再生倒计时期间仍为 true；不得直接把游戏内部 `fullyGrown` 位映射为该字段。`needs_watering` 表示当前作物是否仍需要浇水以推进生长，与当前地块的 `watered` 状态是两个独立事实。

`MachineFact.state` 只在机器运行态可以由当前公开事实一致确认时出现：`IDLE` 表示没有处理中产物，`PROCESSING` 表示仍有正数剩余时间，`READY` 表示当前可以收取；特殊机器或字段彼此不一致时使用 `UNKNOWN`，不得强猜。`UNSPECIFIED` 只保留为 Proto 缺省枚举值，不能由实现显式报告成已确认的机器状态。`input_item` 是游戏仍保留且能安全读取的最近投入物品，`held_item` 是当前预测输出或处理中间产物；部分机器会在原生收取路径中重新决定最终产物，因此调用方不得把 `held_item` 当作已写入背包的最终结果。既有 `ready_for_harvest` 与 `minutes_until_ready` 保持兼容语义，新调用方也不得从缺失的 `state` 反推状态。

`FarmAnimalFact.fullness` 是游戏当前 0..255 饱食值，`fed_today` 必须且只能按 `fullness >= 200` 派生；`auto_petted_today` 与 `petted_today` 分别保留自动和手动照料事实。`produce_ready` 只表示动物身上存在当前可收取产物，不包含已经落在地面的隔夜产物；为 true 且事实可读时，`produce_item_id`、`produce_quality` 与 `produce_harvest_method` 描述该产物及原生收取方式。`age_days`、`adult`、`days_until_mature`、`days_since_last_produce` 与 `base_days_to_produce` 必须来自当前动物实例及其语言无关数据定义，不能从显示名称推断。`has_home_building` 明确区分有无所属建筑；为 true 时 `home_building_id` 使用规范 UUID 字符串，`home_building_type` 使用稳定数据键，`in_home_building` 表示当前是否位于该建筑的室内 Location，不得用建筑显示名或地图短名代替。

`FurnitureFact` 只描述非床家具；床继续使用独立 `BedFact`，不得在家具详情中重复。`qualified_item_id` 是家具实例的 Qualified Item ID；`rotation_count`、`can_rotate`、`seat_capacity` 与 `occupied_seats` 描述当前旋转和座位事实。`has_surface_item` 明确区分表面当前有无物品，为 true 且表面物品可安全读取时提供 `surface_item`；`is_on` 仅在家具存在可安全确认的开关状态时出现。`interaction_kinds` 是语言无关且保守的能力集合，只能包含 `SEAT`、`SURFACE`、`STORAGE` 与 `TOGGLE`，不得包含 `UNSPECIFIED`，也不得把基类 `isActionable` 当作统一交互事实。只有 `interaction_profile_complete=true` 时，空集合才表示本轮已完整判断为普通装饰家具；字段缺失或为 false 时，空集合不得作为不支持交互的负向证据。`storage_item_count` 只在确认存在 `STORAGE` 能力且当前非空物品数量可安全读取时出现。

上述四类详情以及同一 `ItemFact` 子投影必须由 `query_world` 与 `inspect` 复用同一公共投影；给定同一 Ref 和同一游戏事实，两条能力不得返回字段语义不同的副本。所有新增 optional 字段缺失均表示旧实现不支持、事实不适用或本轮无法安全确认；除 `interaction_kinds` 外，调用方不得从 Proto 零值或 JSON 字段缺失推断任何新增事实。

新增派生详情读取失败但核心 typed Fact 仍可安全保留时，World Entity 使用稳定 warning code `ENTITY_FACT_PARTIAL`，Farm Animal 使用 `CHARACTER_FACT_PARTIAL`；warning 的 `ref` 必须指向对应事实，失败的 optional 子字段保持缺省。该状态不得伪装成明确的 false/0，也不得仅因一个派生 getter 失败就丢弃已确认的 Crop/Machine/Furniture/FarmAnimal 核心详情。只有连核心 typed Fact 都无法安全建立时，才继续使用既有 `ENTITY_PROJECTION_FALLBACK` 或 `CHARACTER_PROJECTION_FALLBACK` 规则。
