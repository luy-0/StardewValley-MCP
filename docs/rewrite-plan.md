...
状态：**已完成并封存。** macOS 已完成干净源码与真实游戏验收；Windows 安装路径已静态核对，但按本阶段约定未执行实机验证，后续补测不回开本阶段。
...
1. [x] 在仓库根目录加入 Apache-2.0 `LICENSE`，并在 MCP 等可声明许可证的包元数据中保持一致。
2. [x] 加入 `THIRD_PARTY_NOTICES.md`，按实际分发边界记录第三方组件的名称、版本、许可证、版权归属、来源和是否随产物分发。当前 Mod ZIP 会同时携带 `Google.Protobuf.dll` 及其 BSD-3-Clause 许可文本；包审计会检查项目许可证、第三方声明和许可文件。星露谷物语、SMAPI 和仅用于构建的工具明确列为外部前置条件，不作为本项目发行内容。
3. [x] 在仓库根目录加入中文优先的 `AGENT-GUIDE.md`，覆盖干净源码检出、Windows/macOS 前置环境、Mod 构建安装、MCP 安装、客户端配置、第一次只读调用、第一次受控操作、证据脱敏与故障恢复。
4. [x] 更新面向普通用户的安装文档，分别给出 Windows 与 macOS 的路径和命令写法。两套说明共享同一套能力、契约与验收标准，没有为 Windows 复制实现或协议。
5. [x] 扫描全部公开文档并清理与现状冲突的阶段状态、能力数量、Skill 挂载、MCP Resource/Prompt、安装命令、许可证和历史能力边界表述；公共边界与 Markdown 链接检查通过。
6. [x] 从提交 `abfb2dd` 的干净源码检出完成文档驱动验收：不接触私有仓库即可完成全仓门禁、Mod 源码构建和隔离安装、MCP Tool 安装与 `doctor`，随后在真实存档中通过标准 MCP Session 完成 `query_runtime → face → query_runtime`。十五项 Tool 全部公告，后置查询确认朝向变化且位置未变；公开源码状态保持干净且不包含凭据或机器路径。

本阶段的法律治理只新增 `LICENSE` 与 `THIRD_PARTY_NOTICES.md` 两类根级法律文件，不额外引入安全政策、行为准则或变更日志。暂不提供预编译 Mod 不影响阶段完成；源码生成的 Mod ZIP 与 Python 发行包已经纳入许可证和内容审计。
...
阶段 0—7 已全部完成并封存，当前公开重写计划到此收口。下一步属于正式发布运营，而不是继续扩大重写范围：维护者可以选择预览版本号与发布渠道，创建公开远端、Tag 或 Release；如果开始发布预编译产物，则复用现有包审计并补充下载产物校验。Windows 实机矩阵、可执行 Skill SDK/Host 与新增游戏能力均应按真实需求单独立项，不回填已封存阶段。
