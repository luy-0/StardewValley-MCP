...
状态：**阶段 6 Agent Skill 指引层已完成，下一步进入阶段 7 公开发布准备**
...
状态：**已完成并封存。** 后续如果需要可执行 SDK 或 Skill Host，必须作为新的版本化阶段重新立项，不回填本阶段。

阶段 6 当前采用最简单的 Agent 指引策略：Skill 是标准 `SKILL.md`，由 Codex、Claude 或其他 Agent Runtime 读取，然后直接调用阶段 1—5 已经交付的 MCP Tool。MCP 与 Mod 不加载 Skill，也不增加新 Tool、新协议分支或第二套命令生命周期。

```text
Agent 读取 SKILL.md
        │ 按步骤调用现有 stardew_* Tool
        ▼
...
本阶段只交付四类内容：`spec/skill/` 中的依赖边界说明、一个可复制模板、一个只读示例、一个受控变更示例，以及检查目录、Frontmatter、本仓库安全章节和 Tool 名的静态校验器。五段式正文是 Stardew Valley MCP 示例的编写剖面，不是新的通用 Agent Skill 标准；权限、Ref/Revision、错误和副作用仍完全服从现有 MCP Tool 契约。

1. [x] **阶段 6.0：收缩 Skill 约定。** 删除 `skill.yaml`、`implementation.py` 和可执行插件候选 Schema，改为标准 `SKILL.md` Frontmatter 与本仓库五段式安全编写剖面；明确 Skill 不进入 Proto、Capability Manifest 或 Mod Registry。
2. [x] **阶段 6.1：交付模板与静态校验器。** 模板只含 `SKILL.md` 和可选 `agents/openai.yaml`；校验器使用生成的公共 Tool Catalog 检查 Skill 引用，不连接游戏、不执行工作流，也不建立 SDK。
3. [x] **阶段 6.2：交付两个最小示例。** 只读示例组合 `query_runtime`/`query_world` 生成附近概览；变更示例组合查询、`inspect` 与单次 `use_tool`，通过 Agent 循环复查目标并在 stale、无变化、低体力、错误或次数上限时停止。
4. [x] **阶段 6.3：验证与收口。** 模板和示例通过标准 Skill 校验、仓库静态校验与单元测试；生产源码继续保持没有 Skill Host、动态加载、文件桥或私有平台依赖。

阶段 6 的退出条件是：第三方可以复制模板，只编辑 `SKILL.md` 就描述一条基于公共 MCP Tool 的可复用 Agent 工作流；校验器能够发现无效 Frontmatter、缺失章节和不存在的 Tool；两个示例提供可直接阅读和调用的完整步骤。当前阶段不承诺 Skill 自动安装、自动挂载或确定性代码执行。

此前设计的独立 SDK、可执行 Python Skill、MCP Skill Host、显式挂载与确定性 Harness 保留为未来演进方向。当真实需求证明 Agent 指引不足以支持独立分发或确定性执行时，再建立新的版本化阶段；未来实现仍必须复用现有 Catalog、Command Runtime 与 Transport，不能把编排重新下沉到 Mod。

`ItemGrabMenu` 槽位投影、Inventory Ref/Revision 关联和安全物品转移继续作为公共能力扩展项，不阻塞当前 Agent Skill 层。只有未来编写箱子搬运工作流时，才先扩展版本化公共能力，Skill 不得绕过 MCP 直接操作游戏 UI。

### 阶段 7：公开发布准备

阶段 7 采用“源码优先、Agent 可安装、中文文档优先”的最小发布策略。Windows 与 macOS 同属首批支持范围；SMAPI 和当前实现本身跨平台，因此不另建 Windows 专项开发阶段，也不把 Windows 实机矩阵作为当前发布门禁。公开版本暂不要求提供预编译 Mod 或安装器，用户可以按照源码说明自行编译。

具体任务如下：

1. 在仓库根目录加入 Apache-2.0 `LICENSE`，并在 MCP 等可声明许可证的包元数据中保持一致。
2. 加入 `THIRD_PARTY_NOTICES.md`，按实际分发边界记录第三方组件的名称、版本、许可证、版权归属、来源和是否随产物分发。若未来发布携带 `Google.Protobuf.dll` 的 Mod 压缩包，必须同时携带其 BSD-3-Clause 许可文本；若被分发的 Apache 组件带有上游 `NOTICE`，必须保留其中要求传递的声明。星露谷物语、SMAPI 和仅用于构建的工具应明确列为外部前置条件，不能写成随本项目重新分发的内容。
3. 在仓库根目录加入中文优先的 `AGENT-GUIDE.md`，使任意 Agent 能从干净源码检出开始，检查 Windows 或 macOS 前置环境，编译并安装 Mod，安装 MCP，完成客户端配置，启动游戏，并依次验证第一次只读调用和第一次受控操作。指引必须要求隐藏密钥、避免把本机绝对路径写回公开文件，并给出可复核的成功证据与常见故障恢复步骤。
4. 更新面向普通用户的安装文档，分别给出 Windows 与 macOS 的路径和命令写法。两套说明共享同一套能力、契约与验收标准，不为 Windows 复制一套实现或协议。
5. 扫描仓库内全部公开文档，清理与现状冲突的历史表述，包括旧阶段状态、过期能力数量、“仅支持简单交互”等旧能力边界、失效安装命令、私有平台依赖、机器绝对路径和已删除的 Legacy/V2 架构描述。历史背景只在确有解释价值时保留，并明确标记为历史。
6. 从干净源码检出执行一次文档驱动验收，确认不接触私有仓库即可完成 Mod 源码构建与安装、MCP 安装、第一次只读调用和第一次受控操作；同时检查公开仓库不包含凭据、机器路径、构建垃圾或历史实现。

当前阶段只新增 `LICENSE` 与 `THIRD_PARTY_NOTICES.md` 两项法律文件，不要求额外新增安全政策、行为准则、贡献指南或变更日志。暂不提供预编译 Mod 不影响阶段完成；未来若开始发布二进制 Mod，再把许可证文件随包分发和二进制包审计加入对应发布门禁。

退出条件：Apache-2.0 授权与第三方声明准确；Agent 能只依赖公开仓库和 `AGENT-GUIDE.md` 完成 Mod 与 MCP 的源码安装；Windows 与 macOS 安装说明均可执行；全部公开文档与当前实现一致；发布内容不包含私有路径、凭据、构建垃圾或历史实现。
...
阶段 6 的 Agent Skill 指引层已经完成，下一步进入阶段 7：

1. 加入 Apache-2.0 `LICENSE` 与基于实际分发边界编写的 `THIRD_PARTY_NOTICES.md`。
2. 编写中文优先的 `AGENT-GUIDE.md`，覆盖 Windows 与 macOS 上的 Mod 源码编译安装、MCP 安装、客户端配置、验证和故障恢复。
3. 扫描并更新全部公开文档，删除与当前阶段、能力和架构不一致的历史表述。
4. 从干净源码检出完成一次文档驱动验收；当前不要求提供预编译 Mod 或 Windows 专项实机矩阵。
5. 保留可执行 Skill SDK/Host 为后续独立版本方向，不并入当前发布门禁。
